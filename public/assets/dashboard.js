(function () {
    ...
    // ---------- Time input mask (HH:MM:SS, 24 ชั่วโมง, พิมพ์ทับตำแหน่งเดิม) ----------
    var TIME_MASK = '00:00:00';
    var TIME_DIGIT_POS = [0, 1, 3, 4, 6, 7];

    function nextDigitPos(pos) {
        for (var i = 0; i < TIME_DIGIT_POS.length; i++) {
            if (TIME_DIGIT_POS[i] >= pos) return TIME_DIGIT_POS[i];
        }
        return TIME_DIGIT_POS[TIME_DIGIT_POS.length - 1];
    }

    function prevDigitPos(pos) {
        for (var i = TIME_DIGIT_POS.length - 1; i >= 0; i--) {
            if (TIME_DIGIT_POS[i] < pos) return TIME_DIGIT_POS[i];
        }
        return -1;
    }
    ...
    function handleTimeKeydown(e) {
        ...
        if (value.length !== TIME_MASK.length) value = TIME_MASK;

        var pos = input.selectionStart;

        if (/^[0-9]$/.test(e.key)) {
            e.preventDefault();
            var digitPos = nextDigitPos(pos);
            value = value.slice(0, digitPos) + e.key + value.slice(digitPos + 1);
            var next = digitPos + 1;
            if (TIME_DIGIT_POS.indexOf(next) === -1 && next < value.length) next++;
            input.value = value;
            input.setSelectionRange(next, next);
        } else if (e.key === 'Backspace') {
            e.preventDefault();
            var backPos = prevDigitPos(pos);
            if (backPos === -1) return;
            value = value.slice(0, backPos) + '0' + value.slice(backPos + 1);
            input.value = value;
            input.setSelectionRange(backPos, backPos);
        } else if (e.key === 'Delete') {
            e.preventDefault();
            var delPos = TIME_DIGIT_POS.indexOf(pos) !== -1 ? pos : nextDigitPos(pos);
            value = value.slice(0, delPos) + '0' + value.slice(delPos + 1);
            input.value = value;
            input.setSelectionRange(delPos, delPos);
        } else if (e.key.length === 1) {
            // กันตัวอักษร/สัญลักษณ์อื่นที่ไม่ใช่ตัวเลข ไม่ให้พิมพ์ทับรูปแบบ
            e.preventDefault();
        }
    }
    ...
    function handleTimePaste(e) {
        ...
        var digits = (text.replace(/\D/g, '') + '000000').slice(0, 6);
        e.target.value = digits.slice(0, 2) + ':' + digits.slice(2, 4) + ':' + digits.slice(4, 6);
    }

    timeStart.addEventListener('keydown', handleTimeKeydown);
    timeEnd.addEventListener('keydown', handleTimeKeydown);
    timeStart.addEventListener('paste', handleTimePaste);
    timeEnd.addEventListener('paste', handleTimePaste);
    ...
    function openItemModal(vn, patientLabel) {
        ...
        fetch('/api/vn_items/' + encodeURIComponent(vn))
    ...
    function openCancelModal(vn, patientLabel) {
        ...
        fetch('/api/debt_info/' + encodeURIComponent(vn))
            .then(function (res) { return res.json(); })
            .then(function (data) {
                if (!data.success) {
    ...
    function setAllSelection(checked) {
        ...
        fetch('/api/patients_vns', {
            ...
            .then(function (data) {
                ...
                data.data.forEach(function (row) {
                    selectedVns[row.vn] = { amount: parseFloat(row.debt_amount) || 0, checked: checked };
                });
    ...
    function renderPatients(rows, totalCount, totalAmount, page, pageSize) {
        ...
        rows.forEach(function (row, index) {
            ...
            if (!selectedVns[row.vn]) {
                selectedVns[row.vn] = { amount: parseFloat(row.debt_amount) || 0, checked: true };
            } else {
                selectedVns[row.vn].amount = parseFloat(row.debt_amount) || 0;
            }
            cb.checked = selectedVns[row.vn].checked;
            ...
            btnIssueOne.addEventListener('click', function () {
                openItemModal(row.vn, 'HN: ' + row.hn + '  ชื่อ: ' + row.patient_name + '  VN: ' + row.vn);
            });
    ...
    function renderIssued(rows, totalCount, totalAmount, page, pageSize) {
        ...
        rows.forEach(function (row, index) {
            ...
            btnCancelOne.addEventListener('click', function () {
                openCancelModal(row.vn, 'HN: ' + row.hn + '  ชื่อ: ' + row.patient_name + '  VN: ' + row.vn + '  เลขที่ใบแจ้งหนี้: ' + row.debt_id);
            });
    ...
    function showResults(results) {
        ...
        results.forEach(function (r, index) {
            ...
            tr.appendChild(makeCell(formatVisitDateTime(r.vstdate, r.vsttime)));
            tr.appendChild(makeCell(r.pttype_name || r.pttype));
            tr.appendChild(makeCell(r.auth_code || ''));
